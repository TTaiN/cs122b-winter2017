...
public class MovieViewDB 
{
	...
	{
		ResultSet rs = db.executePreparedStatement("SELECT * FROM movies WHERE id = " + id);
		if (rs.next())
		{
	...
	public LinkedHashMap<Integer, String> getStarsForMovie(int movie_id) throws SQLException
	{
		...
		ResultSet star_list = db.executePreparedStatement("SELECT * FROM stars WHERE stars.id = "
